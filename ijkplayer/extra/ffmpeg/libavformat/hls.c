    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->needed && !pls->pkt.data) {
            while (1) {
                ...
                av_log(s, AV_LOG_INFO, "av_read_frame pls->pkt.dts=%"PRId64", pls->pkt.pts=%"PRId64"\n", pls->pkt.dts, pls->pkt.pts);
                if (ret < 0) {
                ...
                /* Modify packet dts/pts if invalid */
                int seq_no = pls->cur_seq_no - pls->start_seq_no;
                if (pls->pkt.dts != AV_NOPTS_VALUE && seq_no < pls->n_segments) {
                    struct segment *seg = pls->segments[seq_no];
                    if (seg && seg->previous_duration > 0) {
                        // TODO: ugly hack
                        if (pls->pkt.dts >= (1ULL << s->streams[pls->pkt.stream_index]->pts_wrap_bits)) {
                            pls->pkt.dts -= (1ULL << s->streams[pls->pkt.stream_index]->pts_wrap_bits);
                            pls->pkt.pts -= (1ULL << s->streams[pls->pkt.stream_index]->pts_wrap_bits);
                        }

                        int64_t previous_time = seg->previous_duration;
                        if (s->start_time > 0) {
                            previous_time += s->start_time;
                        }
                        previous_time = av_rescale_q(previous_time,
                                            AV_TIME_BASE_Q,
                                            s->streams[pls->pkt.stream_index]->time_base);
                        pls->pkt.dts += previous_time;
                        pls->pkt.pts += previous_time;
                        av_log(s, AV_LOG_INFO, "modified: pls->pkt.dts=%"PRId64", pls->pkt.pts=%"PRId64"\n", pls->pkt.dts, pls->pkt.pts);
                    }
                }

                if (pls->seek_timestamp == AV_NOPTS_VALUE) {
                    // av_log(s, AV_LOG_INFO, "pls->seek_timestamp == AV_NOPTS_VALUE\n");
                    break;
                }
                ...
                if (pls->seek_stream_index < 0 ||
                    ...
                    // av_log(s, AV_LOG_INFO, "pls->seek_stream_index=%d, pls->pkt.stream_index=%d\n", pls->seek_stream_index, pls->pkt.stream_index);
                }
                av_packet_unref(&pls->pkt);
                reset_packet(&pls->pkt);
            }
